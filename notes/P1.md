...
### Testovač
  ...
  - V prípade problémov s prihlasovaním kontaktujte niektorého z cvičiacich.

### Priebeh semestra

  - Predmet sa bude riadiť [pravidlami pre letný
    semester](./Pravidla.html).
  - Okrem bodovaných zadaní budú na testovači zverejňované aj
    nebodované cvičenia, ktorých riešenie je silne odporúčané.

### Softvér potrebný na tomto predmete

  - Java SE 25 (prípadne ľubovoľná verzia od 11 vyššie). Na testovači
...
jazyka Java. Na úvod je užitočné vedieť o tomto jazyku nasledujúce:
  ...
  - Jazyk Java je silne (aj keď nie čisto) *objektovo orientovaný*.
    ...
    OOP – je aj pri tvorbe najjednoduchších možných programov prakticky nevyhnutnosťou (počnúc verziou 25 existuje v Jave možnosť v niektorých prípadoch nutnosť vytvorenia triedy obísť; ide ale o prakticky nepodstatnú kuriozitu).
...
nej](https://docs.oracle.com/en/java/javase/25/docs/api/index.html).
...
[`equals`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/String.html#equals(java.lang.Object)).
...
reťazcami, spomeňme aspoň tieto:
  ...
  - Metóda
    [`length`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/String.html#length) vráti dĺžku daného reťazca. *Pozor*: keďže ide o metódu
    (bez argumentov), na rozdiel od polí pre reťazec `s` píšeme
    `s.length()` so zátvorkami na konci.
  - Metóda
    [`charAt`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/String.html#charAt(int))
...
[`PrintStream`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/io/PrintStream.html),
...
[`File`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/io/File.html)
...
[`close`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/Scanner.html#close()).
...
dokumentácii](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/Scanner.html).
Tu spomeňme aspoň nasledujúce metódy:
  ...
  - `nextLine`: prečíta riadok až po jeho koniec a výsledný reťazec (bez
    prípadného nového riadku na konci) vráti na výstupe.
...
Trieda `BufferedReader` pritom poskytuje dve kľúčové metódy:
  ...
  - `readLine`: prečíta jeden riadok zo vstupu; jej návratový typ je
    `String` (reťazec bez prípadného nového riadku na konci).
